...
class modVisualizer:
    ...
    def draw_instance_predictions(self, predictions, partsList): # predicted results and parts classes to be removed
        """
...
#########################################################################################################
        # Remove parts masks and keep only damage masks
        
        rMaskIndex = []
        if partsList:        
            classeslist = classes.tolist()
            for i in range(len(classeslist)):
                if classeslist[i] in partsList:
                    rMaskIndex.append(i)                    
            [masks.pop(i) for i in reversed(rMaskIndex)]      
    ...
    def overlay_instances(
        ...
        *,
        ...
        if assigned_colors is None:
            ...
            #assigned_colors = [(1.0, 0.0, 0.0) for _ in range(num_instances)] ################################### const. color Red

            assigned_colors = [np.random.rand(3) for _ in range(num_instances)] ################################### Random color value
        if num_instances == 0:
            return self.output
